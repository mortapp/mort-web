...
function Fragment({ index, drag }: { index: number; drag: RefObject<DragState> }) {
  ...
  useBeforePhysicsStep(() => {
    ...
    rb.addForce({ x: (anchor.x - p.x) * .85 - v.x * .3 + Math.sin(time.current*.7+index)*.055, y: (anchor.y - p.y) * .85 - v.y * .3 + Math.sin(time.current*.9+index)*.045, z: (anchor.z - p.z) * .85 - v.z * .3 }, true)
    ...
    rb.applyImpulse({ x: d.vx * influence * .16, y: -d.vy * influence * .16, z: -d.energy * influence * .08 }, true)
    rb.applyTorqueImpulse({ x: d.vy * .006, y: d.vx * .006, z: d.energy * .004 }, true)
    if (index === 0) document.querySelector<HTMLElement>('.mort-world')?.setAttribute('data-physics-impulse', (Math.hypot(d.vx,d.vy)*influence*.16).toFixed(5))
  })
